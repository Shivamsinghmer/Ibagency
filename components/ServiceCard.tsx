...
export default function ServiceCard({ service, index }: ServiceCardProps) {
  ...
  return (
    <div 
      className={`
        rounded-2xl p-8 md:p-10 transition-all duration-300 hover:translate-y-[-4px] group cursor-pointer flex flex-col justify-between
        ${isDark ? 'hover:shadow-[0_8px_40px_rgba(255,255,255,0.06)]' : 'hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)]'}
        ${service.bg}
        ${service.full ? 'md:col-span-2' : ''}
        ${service.featured ? 'min-h-[300px]' : 'min-h-[260px]'}
      `}
      style={{ '--delay': `${index * 50}ms` } as any}
    >
      <div>
        {Icon && (
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-6 transition-colors ${isDark ? 'bg-white/5 border border-white/10 text-white/80 group-hover:bg-white/10 group-hover:text-white' : 'bg-fg/5 border border-fg/10 text-fg'}`}>
            <Icon size={24} strokeWidth={1.5} />
          </div>
        )}
        <h3 className={`font-bricolage font-semibold text-[20px] md:text-[22px] mb-3 tracking-tight ${isDark ? 'text-white' : 'text-fg'}`}>
          {service.title}
        </h3>
        <p className={`text-[14px] md:text-[15px] leading-relaxed line-clamp-3 mb-6 ${isDark ? 'text-white/60 group-hover:text-white/80 transition-colors' : 'text-muted'}`}>
          {service.desc}
        </p>
      </div>
